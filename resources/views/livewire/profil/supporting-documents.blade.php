<div>
    <section class="pt-30 lg:pt-25">
        <div 
            style="background-image: url('{{ asset('storage/' . $supportingContent['hero']['image']) }}')"
            ...
            <div class="relative z-2 w-full h-full flex flex-col justify-center mx-0 lg:mx-14">
                ...
                <h1 class="heading-48s text-bkkNeutral-50 mb-3 lg:w-[55%]">
                    {{ $supportingContent['hero']['title'] }}
                </h1>
                <div class="paragraph-16r text-bkkNeutral-100 w-full lg:w-[60%]">
                    {{ $supportingContent['hero']['description'] }}
                </div>
            </div>
        </div>
    </section>
    <section class="py-15 md:py-20">
        <div 
            x-data="{
                selectedContent: '{{ $supportingItems[0]['data']['title'] }}',
                 init() {
            ...
            <div 
                ...
                @foreach ($supportingItems as $document)
                    <div 
            ...
            <div class="w-full lg:w-[75%]">
                @foreach ($supportingItems as $document)
                    <div 
                        x-cloak
                        x-show="selectedContent === '{{ $document['data']['title'] }}'">
                        {{-- Sarpras --}}
                        @if ($document['type'] === 'sarana_prasarana')  
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    @foreach ($document['data']['device_items'] as $device )
                                        <div class="bg-white shadow-lg overflow-hidden rounded-[20px] my-2">
                                            <div class="w-full h-[180px]">
                                                <img 
                                                    ...
                                                    : asset('/assets/static/fallback/fallback-image.webp')
                                                    }}" />
                                            </div>
                        ...
                        <div>
                            ...
                            <div>
                                <iframe 
                                    ...
                                    src="{{ $document['data']['link_gdrive'] }}" allowfullscreen>
                                </iframe>
                            </div>
                        </div>
                        @endif
